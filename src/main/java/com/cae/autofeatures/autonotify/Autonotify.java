...
public class Autonotify {
    ...
    private static void notify(Notification notification){
        ...
        AutonotifyProvider.SINGLETON
                .getSubscribers()
                .forEach(notificationObserver -> CompletableFuture.runAsync(() -> notificationObserver.receiveNotification(notification), executor));
    }
    ...
    private static void checkExceptionsCriteria(Exception actualException, ArrayList<String> reasons) {
        ...
        if (notificationSettings.getExceptionsToConsider().contains(actualException.getClass()))
    ...
    private static boolean checkWhetherGenericallyMatchesMappedExceptions(
        ...
        var map = notificationSettings.getExceptionsToConsider();
        return
